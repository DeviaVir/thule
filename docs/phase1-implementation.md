...
- changed-file project discovery (`thule.conf` rooted projects),
...
3. **YAML config support**
   - config loader now decodes JSON first and falls back to YAML parsing for advertised `thule.conf` files.
